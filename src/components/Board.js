    ...
    return (
        <div className="board-container">
            <div className={`board ${winnerEffect}`}>
                ...
                <div className="board-ball" style={{rotate: rotate + "deg"}}>
                    <div className="ball" style={{opacity: hideBall ? "0" : "1"}}></div>
                </div>
                <div className={playable ? "spin-container" : "spin-container disabled-btn"}>
                    <button type="button" className="spin" onClick={()=>play()}>PLAY</button>
                </div>
                    ...
                    <div className="innerCircle">
                        {
                            boardItems.map((elm, index) => {
                                return (
                                    <div 
                                        key={index} 
                                        className={showItemBall && elm.number === winnerNumber ? elm.class + " activeItem" : elm.class} 
                                        style={{ top: elm.topStyle, left: elm.leftStyle, rotate: elm.rotStyle }}
                                    >
                                        <span>{elm.number}</span>
                                        <div className="board-item-top a"></div>
                                    </div>
                                )
                            })
                        }
